...
class OrganizationService:
    ...
    async def create_organization(
        ...
        query = """
            INSERT INTO organizations (
                ...
                :uuid, :name, :display_name, :description, :slug, :website,
                :billing_email, 'free', true, :created_by,
        ...
        org = await self.db.fetch_one("SELECT * FROM organizations WHERE uuid = :uuid", {"uuid": org_uuid})
        return org
    ...
    async def _generate_slug(self, name: str) -> str:
        ...
        original_slug = slug
        while True:
            existing = await self.db.fetch_one("SELECT id FROM organizations WHERE slug = :slug", {"slug": slug})
            if not existing:
                break
    ...
    async def get_organization(self, org_id: int):
        ...
        query = "SELECT * FROM organizations WHERE id = :id AND is_active = true"
        return await self.db.fetch_one(query, {"id": org_id})
    ...
    async def get_user_organizations(self, user_id: int):
        ...
        query = """
            ...
            WHERE om.user_id = :user_id AND om.status = 'active' AND o.is_active = true
            ORDER BY o.name
        """
    ...
    async def update_organization(self, org_id: int, update_data: Dict[str, Any]):
        ...
        if set_clauses:
            ...
            query = f"UPDATE organizations SET {', '.join(set_clauses)} WHERE id = :id"
    ...
    async def add_member(self, organization_id: int, user_id: int, role: str, invited_by: int):
        ...
        query = """
            ...
            ) VALUES (:organization_id, :user_id, :role, 'active', :invited_by, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
            ON CONFLICT (organization_id, user_id) DO UPDATE SET
                role = EXCLUDED.role, status = 'active', invited_by = EXCLUDED.invited_by, updated_at = CURRENT_TIMESTAMP
        """
    ...
    async def get_member(self, organization_id: int, user_id: int):
        ...
        query = """
            ...
            WHERE om.organization_id = :organization_id AND om.user_id = :user_id AND om.status = 'active'
        """
        return await self.db.fetch_one(query, {"organization_id": organization_id, "user_id": user_id})
    ...
    async def get_member_count(self, organization_id: int) -> int:
        ...
        """
        result = await self.db.fetch_one(query, {"organization_id": organization_id})
    ...
    async def remove_member(self, organization_id: int, user_id: int):
        ...
        await self.db.execute("""
            ...
            WHERE user_id = :user_id AND team_id IN (
                SELECT id FROM teams WHERE organization_id = :organization_id
            )
    ...
    async def create_invitation(
        ...
        query = """
            INSERT INTO invitations (
                ...
                :uuid, :organization_id, :email, :role, :team_id, :message, :token,
                'pending', :invited_by, :expires_at, CURRENT_TIMESTAMP
            )
        """
        ...
        invitation = await self.db.fetch_one(
            "SELECT * FROM invitations WHERE uuid = :uuid", {"uuid": invitation_uuid}
        )
        return invitation
    ...
    async def create_team(
        ...
        team = await self.db.fetch_one("SELECT * FROM teams WHERE uuid = :uuid", {"uuid": team_uuid})
        return team
    ...
    async def add_team_member(self, team_id: int, user_id: int, role: str = "member", added_by: int = None):
        ...
        query = """
            ...
            ) VALUES (:team_id, :user_id, :role, :added_by, CURRENT_TIMESTAMP)
    ...
    async def get_team_member(self, team_id: int, user_id: int):
        ...
        """
        return await self.db.fetch_one(query, {"team_id": team_id, "user_id": user_id})
    ...
    async def remove_team_member(self, team_id: int, user_id: int):
        ...
        await self.db.execute(query, {"team_id": team_id, "user_id": user_id})
    ...
    async def get_recent_activity(self, organization_id: int, limit: int = 10):
        ...
        query = """
            ...
            LIMIT :limit
        """
        return await self.db.fetch_all(query, {"organization_id": organization_id, "limit": limit})
